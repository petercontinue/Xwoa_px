...
public interface IPxstafftableDao extends BaseMapper<Pxstafftable> {
    ...
    @Select("<script>" +
            "SELECT a.id id,concat(concat('(','', b.campusName),'',concat(')',' ', a.staffName)) name from pxstafftable a" +
            ...
            "</if>"
            + "</script>")
    List<listVo> GetBanzhurenFenpei(@Param("ew") QueryWrapper queryWrapper);
}