 ...
 */
@Repository
public interface IPxassetsaddtableDao extends BaseMapper<Pxassetsaddtable> {
    ...
    @Select("<script>" +
            "SELECT * from  pxassetsaddtable"
            + "</script>")
    List<Pxassetsaddtable> getAllList();

    @Select("<script>"+"SELECT b.assetsName,c.assetsName AS styleName,a.num,a.beizhu,d.staffName AS jinbanStaffName,a.addTime FROM pxassetsaddtable AS a \n" +
            ...
            "</if>"
            + "</script>")
    Page<assetAddVO> GetassetsAddPages(Page page, @Param("ew") Wrapper wrapper);
}