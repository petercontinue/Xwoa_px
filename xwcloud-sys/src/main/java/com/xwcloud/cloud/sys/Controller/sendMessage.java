    ...
    public boolean sendMessage(Long tuisongtableID) {
        ...
        if (pxtuisongtable.getRole() == 1) {//推送学生消息
            ...
            if (pxtuisongtable.getTuisongTypeName()==5) {//推送消课通知
                ...
                String classWarning = iPxsysparamvaluetableService.GetPxsysparamvalueByQiyeIDAndValueID(pxtuisongtable.getQiyeID(), 13L) == null ? iPxsysparamdefaulttableService.getById(13).getDefaultValue() : iPxsysparamvaluetableService.GetPxsysparamvalueByQiyeIDAndValueID(pxtuisongtable.getQiyeID(), 13L).getModifyValue();
                if (classWarning == "总课时预警") {
                    stuKechengRemainKeshi = iPxbuxikechengtableService.getSumzongRks(pxtuisongtable.getStuID(),pxtuisongtable.getQiyeID()).get(0).getSumR();
                } else {
            ...
            (pxtuisongtable.getTuisongTypeName()==4) {//积分变动提醒
            ...
            (pxtuisongtable.getTuisongTypeName()==2) {//到校签到
